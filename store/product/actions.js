export default {
  ...
  async SET_DETAIL({commit}, detail) {
    if(!detail) {
      ...
      commit('CHANGE_DETAIL', null)
      await this.$axios.$get(`${this.$api.advertises.index}/${detail}?with=category;author.avatar;gallery;city;answers.filter`)
  ...
  async SET_RECOMEND({commit, state},category_id) {
    ...
    await this.$axios.$get(`${this.$api.advertises.index}?${sort}&searchJoin=and&with=category;author;gallery;city;answers&page=1&per_page=6&search=status:0,1;category_id:${category_id}`)
      .then(({data,meta}) => {
