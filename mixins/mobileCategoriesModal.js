...
export default {
  ...
  computed: {
    ...
    slugRoute() {
      return this.$changeRoute(`/catalog/${this.slugs.join('/')}`)
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    openModal() {
      if (this.active) {
        this.oldIndex.push(this.active - 1)
    ...
    changeActiveCategory(index) {
      if (this.activeCategory) {
        this.activeCategory = this.activeCategory.allSubCategories[index]
        this.$router.push(this.slugRoute)
      }
      else {
        this.activeCategory = this.CATEGORIES[index]
        this.slugs.push(this.activeCategory.slug)
        if (this.activeCategory.allSubCategories.length === 0) this.$router.push(this.slugRoute)
      }
    }
  }
}
