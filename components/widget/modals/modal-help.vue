<template lang="pug">
  b-modal(
    ...
    form(  ref="send-form" )
      h3.form-contact.h3 Need our help?
      .form-contact__fields
        .form-contact__field
          .form-contact__label.h4 Title
          form-input.form-contact__input(
            :placeholder="'Title'" name="name" v-model="form.name"
        ...
        <!--.form-contact__field(v-if="$auth.user.loggedIn")-->
          ...
          <!--)-->
        .form-contact__field
          .form-contact__label.h4 Choose theme
          form-select-themes.form-contact__select(@input="(val) => theme = val" v-model="theme" :items="themes"  default)
        .form-contact__field
          .form-contact__label.h4 Enter your question
          form-input.form-contact__input(
            :placeholder="'Enter your question'" name="description" textarea v-model="form.description"
      ...
      .form-contact__bottom.flex.flex_vertical.flex_justify
        .form-contact__buttons.flex
          button-primary.form-contact__btn-send(blue @click.prevent.native="sendForm" :loading="loading") Send
  ...
  methods: {
    ...
    sendForm() {
      ...
      if (!this.$v.form.$error && !this.apiError) {
        ...
        this.$axios.$post(this.$chat.addTicket, formData).then(res => {
          this.form = {
            // email: '',
            description: '',
          }
