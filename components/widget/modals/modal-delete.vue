<template lang="pug">
  b-modal(
    ...
    div(:class="status")
      ...
      .modal-delete__btns
        button-primary.modal-delete__btn( @click.native.prevent="closeModal") {{POPUP_GLOBAL.close}}
        button-primary.modal-delete__btn.text-capitalize(
          v-if="!withOutBtn" :class="[{blue:status === 'activate' || status === 'moderation'},{red:status !== 'activate' || status !== 'moderation'}]"   @click.native.prevent="deleteItem") {{btn}}
</template>
