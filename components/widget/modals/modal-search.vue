<template lang="pug">
  b-modal(
    ...
    form.modal-search__form
      .modal-search__field
        form-input.modal-search__input(:placeholder="placeholder" v-model="search"  @touchstart="initiateKeyboard"   ref="searchBarButton")
      ...
      .modal-search__cancel-wrap
        .modal-search__cancel(@click='closeModal()') {{PAGE_HOME.searchModal.cancel}}
        ...
        .modal-categories__list.p-0
          .modal-categories__item.mt-0(v-if="slugs.length === 0" @click="showAllCategory")
            span.modal-categories__link
              span.modal-categories__link-text.modal-categories__link-text__grey {{PAGE_HOME.allCategories}}
              svg-icon.modal-categories__link-arrow(name="arrowRight")
          .modal-categories__item.mt-0(v-if="slugs.length > 0" @click="showAllCategory")
            span.modal-categories__link
              span.modal-categories__link-text.modal-categories__link-text__grey All about {{activeCategory.name}}
              svg-icon.modal-categories__link-arrow(name="arrowRight")
    ...
    .modal-search__container-1.modal-categories__item-black(v-else)
      .modal-categories__item.mt-0(
        ...
        NuxtLink.modal-categories__link.modal-categories__link-data(:to="getRouter(item,item.product_name)")
          span
      ...
      .modal-categories__item.mt-0(
        ...
        NuxtLink.modal-categories__link(:to="$changeRoute(`/catalog?search_text=${item.product_name}`)")
          span
      ...
      .modal-categories__item.mt-0(
        ...
        NuxtLink.modal-categories__link(:to="$changeRoute(`/catalog?search_text=${search}`)")
          span
            span.modal-categories__link-text
              | All Categories
...
export default {
  ...
  methods: {
    openModal() {
      if (this.$route.query.search_text) this.search = this.$route.query.search_text
     setTimeout(()=>{
       this.focusElement()
     },200)
    },
    ...
    getRouter(item,product_name) {
      ...
      return this.$changeRoute(`/catalog/${slug}?search_text=${product_name || this.search}`);
    },
    focusElement() {
