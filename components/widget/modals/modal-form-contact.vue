  ...
  b-modal(
    ...
    form(v-if="POPUP_CONTACTS" ref="send-form" )
      ...
      transition(name="page")
        .form-contact__field
          .alert-error(v-if="apiError") Have any Errors
      .form-contact__fields
        .form-contact__field
          .form-contact__label.h4 {{POPUP_CONTACTS.name.label}}
          form-input.form-contact__input(
            :placeholder="POPUP_CONTACTS.name.placeholder" name="name" v-model="form.name"
        ...
        .form-contact__field
          ...
          form-input.form-contact__input(
            :placeholder="POPUP_CONTACTS.email.placeholder" name="email" type="email" v-model="form.email"
        ...
        .form-contact__field
          ...
          form-input.form-contact__input(
            :placeholder="POPUP_CONTACTS.question.placeholder" name="description" textarea v-model="form.question"
      ...
      .form-contact__bottom.flex.flex_vertical.flex_justify
        .form-contact__buttons.flex
          button-primary.form-contact__btn-send(blue @click.prevent.native="sendForm" :loading="loading") {{POPUP_CONTACTS.send}}
  ...
  methods: {
    ...
    sendForm() {
      ...
      if (!this.$v.form.$error && this.files.length <= 5 && !this.apiError) {
        ...
        this.loading = true
        this.$axios.$post(this.$api.support.add, formData).then(res => {
          this.closeModal()
