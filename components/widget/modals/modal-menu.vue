<template lang="pug">
  b-modal(
    ...
    @hide="closeModal"
    centered
    entrance-transition="animated fadeIn"  leave-transition="animated bounceOut"
  )
    ...
    .modal-menu__list.modal-menu__list-1
      .modal-menu__item(v-for="(item,i) in menu" :key="i" v-if="!item.isUser || $auth.loggedIn === true && item.isUser" :class="`btn-item-${item.icon}`")
        button-border.w-full(:icon="item.icon" :to="$changeRoute(item.to)" :class="{'button-nav_notif' : item.notif}")  {{item.text}}
      .modal-menu__item
        button-border.w-full(icon="lang" v-b-modal.modal-languages)  {{PAGE_MODAL_MENU.lang}}
      .modal-menu__item
        button-border.w-full(v-if="$auth.loggedIn" red icon="exit" @click.native.prevent="logOut")  {{PAGE_MODAL_MENU.exit}}
    template(v-if="!$auth.loggedIn")
      ...
      .modal-menu__list
        ...
        .modal-menu__item
          button-primary.w-full(light :to="$changeRoute('/auth/registration')")  {{PAGE_MODAL_MENU.register}}
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getPageTranslate() {
      if(!this.$screen.ld) this.SET_PAGE_MODAL_MENU()
    },
    logOut() {
