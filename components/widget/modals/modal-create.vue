<template lang="pug">
  ...
  @hide="closeModal"
      ...
      transition(name="fade")
        form.modal-create__search(action="/#search" v-if="search")
          .modal-create__search-field
            form-input.modal-create__search-input(:key="keySearch" placeholder="Search" v-model="searchText" ref="InputSearch")
    ...
    .modal-create__categories.css-scrollbar(v-if="other && !search")
...
export default {
  ...
  computed: {
    categoryList() {
      ...
      if (this.activeCategory && this.searchText ==='') return this.activeCategory.allSubCategories
  ...
  methods: {
    ...
    openModal() {
      ...
      this.search = false
      this.searchText = ''
      // if(this.categoryParentId !== 0 && this.editCategory){
      //   this.other = false
      //   this.activeCategory = null
      //   this.bred = []
      //   let withFields = `with=picture;allSubCategories;parentCategories.allSubCategories.subCategories.parentCategories`
      //   this.$axios.$get(`${this.$api.categories}?searchFields=id:=&search=id:${this.categoryParentId}&${withFields}`)
      //     .then(({data}) => {
      //       this.activeCategory = data[0].parentCategories[0]
      //       console.log(data[0].parentCategories[0].allSubCategories[0].parentCategories[0].allSubCategories,'data[0].parentCategories[0].allSubCategories')
      //       this.bred = this.$categoryBreadcrumbs(data[0])
      //       console.log(this.bred,'bred')
      //     })
      // }else{
