    ...
    form(ref="send-form" )
      ...
      .form-contact__fields
        ...
        .form-contact__field
          ...
          form-select-themes.form-contact__select(@input="(val) => form.theme = val" v-model="form.theme" :items="themes"  default)
        .form-contact__field
          .form-contact__label.h4 {{POPUP_SUPPORT.question.label}}
          form-input.form-contact__input(
            :placeholder="POPUP_SUPPORT.question.placeholder" name="description" textarea v-model="form.question"
      ...
      .form-contact__bottom.flex.flex_vertical.flex_justify
        .form-contact__buttons.flex
          button-primary.form-contact__btn-send(blue @click.prevent.native="sendForm" :loading="loading") {{POPUP_SUPPORT.send}}
          button-file.form-contact__btn-file(name="files[]" @changeFiles="changeFilesHear()")
  ...
  methods: {
    ...
    sendForm() {
      ...
      if (!this.$v.form.$error && !this.apiError) {
        ...
        formData.append('support_theme_id', this.form.theme ? this.form.theme.id : 1)
        if(this.$auth.loggedIn)   formData.append('support_theme_id', this.form.theme ? this.form.theme.id : 1)
        if (this.$auth.loggedIn) this.$axios.setHeader('Authorization', `Bearer ${this.$auth.strategy.token.get()}`);
        this.$axios.$post(this.$api.support.add, formData).then(() => {
          this.closeModal()
          this.$root.$emit('bv::show::modal', 'modal-thank');
          if (this.$auth.loggedIn) {
            setTimeout(()=>{
              this.$root.$emit('bv::hide::modal', 'modal-thank');
              this.$router.push(this.$changeRoute('/chat?tab=support'));
            },2000)
          }
        }).catch(e => {
          this.apiError = true
