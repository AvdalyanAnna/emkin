  ...
  b-modal(
    ...
    template(#modal-footer='{ close }')
      .modal-location__buttons.flex
        ...
        button-primary.modal-location__btn(small :loading="loading" blue @click.native='save()') Send
...
export default {
  ...
  methods: {
    ...
    send() {
      ...
      let api = this.$api.changeSettings.password
      if (!this.changeUserInfo) {
        info.confirmation_auth = this.$auth.user.email || this.$auth.user.phone
        api = this.$api.changeSettings.email
      }
      this.$axios.$put(api, info)
          .then(() => {
            this.closeModal()
    ...
    save() {
      if (this.codeString.length !== this.amount && !this.loading) this.error = true
      else this.send()
    }
  }
}
