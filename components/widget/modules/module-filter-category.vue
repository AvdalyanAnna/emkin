<template lang="pug">
  .filter-category
    .filter-category--header(v-if="!mobile")
      span.filter-category--header_title( @click="changeCategory(`/catalog`)") All categories
      ...
      .filter-category--item(v-for="item in categoriesList" :key="item.id")
        .filter-category--item_info(:class="{active:actives[0] === item.id || shows[0] === item.id}")
          span.filter-category--item_inner
            .filter-category--arrow( @click="changeCategory(`/catalog/${item.slug}`,item,0)")
        ...
        transition-group(name="slide"  v-on:before-enter="beforeEnter" v-on:enter="enter"
          ...
          .filter-category--item.filter-category--item_child(
            ...
            .filter-category--item_info(:class="{active:actives[1] === item1.id || shows[1] === item1.id  }")
              span.filter-category--item_inner
                .filter-category--arrow(:class="{'opacity-0':item1.allSubCategories.length === 0}"  @click="changeCategory(`/catalog/${item.slug}/${item1.slug}`,item1,1)")
            ...
            transition-group(name="slide" v-on:before-enter="beforeEnter" v-on:enter="enter"
              ...
              .filter-category--item.filter-category--item_child(
                ...
                .filter-category--item_info(:class="{active:actives[2] === item2.id || shows[2] === item2.id}")
                  span.filter-category--item_inner
                    .filter-category--arrow(:class="{'opacity-0':item2.allSubCategories.length === 0}"  @click="changeCategory(`/catalog/${item.slug}/${item1.slug}/${item2.slug}`,item2,2)")
                ...
                transition-group(name="slide" v-on:before-enter="beforeEnter" v-on:enter="enter"
                  ...
                  .filter-category--item.filter-category--item_child(
                    ...
                    .filter-category--item_info(:class="{active:actives[3] === item3.id || shows[3] === item3.id}")
                      span.filter-category--item_inner
                        .filter-category--arrow(:class="{'opacity-0':item3.allSubCategories.length === 0}"  @change="changeCategory(`/catalog/${item.slug}/${item1.slug}/${item2.slug}/${item3.slug}`,item3,3)" )
...
export default {
  ...
  methods: {
    getActiveCategories() {
      let route = ''
      route = this.routeSlug ? this.routeSlug.split('/') : this.$route.path.split('/')
    ...
    changeCategory(slug,category,index){
      if(this.mobile){
        this.$emit('changeActive',{...category,to:this.$changeRoute(slug)})
        this.changeShow(category.id,index)
      }else{
        const query = decodeQuery(this.$route.query.data)
        if(query.answersFiltersIds) delete query.answersFiltersIds
        this.$router.push(`${this.$changeRoute(slug)}?${encodeQuery(query)}`)
      }
    }
  }
}
