<template lang="pug">
  section-registration.confirm-page(v-if="PAGE_REGISTER_CONFIRM" :title="isEmail ? PAGE_REGISTER_CONFIRM.titleEmail : PAGE_REGISTER_CONFIRM.titlePhone" :img="{left:'reset1.png',right:'reset2.png'}" )
    ...
    template
      .c-pointer.registration-form__desc.mobile-hidden.p.color-blue(
        v-if="currentTime === 0" @click="resendCode"
      )
        | Resend code
    ...
    .registration-form__social(v-if="link")
      .registration-form__social-text.p Go to the mail:
      .registration-form__social-list.flex
        button-primary.registration-form__social-btn.registration-form__social-btn-100(
          gray :to="link.target_url" link target="_blank" :icon="link.icon_name"
        )
          | {{ link.title }}
    template(v-slot:bottom)
      button-primary.registration-form__btn(@click.prevent.native="confirm($event)" blue) {{PAGE_REGISTER_CONFIRM.button}}
      template
        button-primary.c-pointer.registration-form__btn.mobile-visible(
          v-show="currentTime === 0" @click="resendCode" gray
        )
          span.color-gray Resend code
        button-primary.registration-form__btn.mobile-visible(:loading="loadingBtn" gray v-show="currentTime !== 0")
...
export default {
    ...
    getDesc() {
      ...
      if (this.isEmail) {
        ...
        if (this.currentTime === 0) {
          ...
          html += `"${this.PAGE_REGISTER_CONFIRM.codeResetText} ${this.$getMinutes(this.currentTime)}"`
        }
        html+=`${this.PAGE_REGISTER_CONFIRM.isEmail.desc3} <a href="mailto:Help56@mail.ru">Help56@mail.ru</a>`
      } else {
    ...
    if (!this.modal) {
      if (!this.USER) {
        ...
        if (token) {
          ...
          if (user) {
            this.SET_USER(user)
            if (!this.$validateEmail(user.confirmation_auth)) this.isEmail = false
          }
        } else {
          this.$goRegister()
        }
  ...
  methods: {
        ...
        try {
          ...
          if (res.status === 'Success') {
            if (this.reset) {
              ...
              this.$router.push(this.$changeRoute('/auth/reset-password'))
              this.loadingBtn = false
            } else {
              this.$auth.strategy.token.set(res.data.access_token || null)
              this.$auth.setUser(res.data.user)
              await this.$auth.fetchUser().then(() => this.$router.push(this.$changeRoute('/auth/registration-done')))
    ...
    resendCode(e) {
      ...
      this.$axios.$post(this.$authApi.confirmCode, form).then(res => {
