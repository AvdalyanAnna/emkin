<template lang="pug">
  .check-field(:class="[{error:error},{valid:valid}]")
    .check-field__first
      button-primary(:white="activeInput !== 'email'" @click.prevent.native="activeInput = 'email'") {{email}}
      button-primary(:white="activeInput !== 'phone'" @click.prevent.native="activeInput = 'phone'") {{phoneText}}
    template(v-if="activeInput === 'phone'")
      .check-field__content.check-field__content-phone
        ...
        .check-field__phone
          ...
          input.check-field__phone-input(type="tel" :placeholder="activePhone.phone"  v-model="myInputModel" v-mask="activePhone.phone")
    template(v-else)
      .check-field__content
        .check-field__email
          input.check-field__phone-input(type="email" :placeholder="placeholder"  v-model="myInputModel")
    .check-field__error.color-red(v-if="errorText !== ''") {{ content[errorText]}}
...
export default {
  ...
  watch: {
    ...
    myInputModel(val) {
      ...
      this.isValidate(val)
      if (this.activeInput !== 'email' && isValidPhoneNumber(this.activePhone.code + val, this.activePhone.prefix)) {
        if (this.check) {
          ...
          this.$axios.$get('/auth/check-exists', {params: form})
              ...
              .finally(() => this.$emit('input', `${this.activePhone.code}-${val}`))
        } else {
  ...
  methods: {
    ...
    getLocation() {
      this.$axios.$get('http://www.geoplugin.net/json.gp').then(res => {
    ...
    isValidate(val) {
      if (this.activeInput === 'email') {
        this.$emit('error', !this.$validateEmail(val))
      } else {
        this.$emit('error', !isValidPhoneNumber(this.activePhone.code + val, this.activePhone.prefix))
      }
    }
  }
}
