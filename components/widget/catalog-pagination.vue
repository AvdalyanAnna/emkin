  ...
  .catalog-pagination(v-if="total > 1")
    ...
    div(v-for="i in total" :key="i" )
      ...
      .catalog-pagination-item(v-else-if="i >= start && i < start + count " :class="{active: i === page}" @click="changePage(i)")
        span {{i}}
      .catalog-pagination-more(v-else-if="i === start + count  && i !== total" @click="changePage(i)")
        span -
      .catalog-pagination-item(v-else-if="i === total ")
        span {{i}}
...
export default {
  ...
  computed: {
    ...
    minus(){
      if(this.$screen.width < 768) return this.count  - 1
      return this.count  - 2
    },
    start() {
  ...
  methods: {
    changePage(page) {
      if (page !== false) {
        ...
        let query = decodeQuery(this.$route.query.data)
        query.page = page
