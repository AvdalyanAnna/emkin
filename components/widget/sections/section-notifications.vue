<template lang="pug">
  section.section-notifications
    ...
    part-back-nav.section-notifications__nav(:nav="PAGE_NOTIFICATIONS.nav" :keyActive="activeKey" activeRouterClass="" mobile)
    ...
    .section-notifications__list(v-if="notificationsLang.length > 0")
      item-notification.section-notifications__item(
        ...
        @actions="actionsElement(item.id,i)"
      )
      infinite-loading(:distance="20" ref="infiniteLoading" @infinite="infinite")
        div(slot="no-results")
        div(slot="no-more")
        div(slot="spinner")
    .section-notifications__center(v-else)
      part-not-notification.section-notifications__not-notification {{text}}
...
export default {
    ...
    notificationsLang() {
      ...
      newNotifications.forEach(item => {
        ...
        data.forEach(([key,value]) => {
          if (value && typeof value === 'object') notification.data[key] = value[lang] || value['en']
          else notification.data[key] = value
        })
  ...
  methods: {
    ...
    actionsElement(id, index) {
      if (!this.NOTIFICATIONS[index].read_at) {
        this.SET_ACTIONS_NOTIFICATION({id, index, slug: this.$route.params.slug})
        setTimeout(()=>
