        ...
        .section-catalog__row.mt-0.pt-0
          ...
          .section-catalog__column.section-catalog__column_right
            .section-catalog__nav.flex.flex_vertical.flex_justify
              button-filter.section-catalog__btn-filter(:text="PAGE_CATALOG.filters" :counter="counter" @click.native="openFilter()")
              form-select-sort-index.favorites-ad__select( :key="sortKey" :label="PAGE_CATALOG.sortBy" :items="sortList" v-model="sort" @input="changeSort" :value="sort" noBorder)
    ...
    .section-catalog__wrap.wrap
      .section-catalog__row.mt-0
        ...
        .section-catalog__column.section-catalog__column_right(:class="{full:full}")
          ...
          //    )
          .section-catalog__column.section-catalog__column_right-inner
            .section-catalog__loading(v-if="PRODUCTS_LOADING && PRODUCTS.length === 0")
              loader(loaderType="white")
            list-null(v-else-if="PRODUCTS.length === 0" )
            div(v-else )
...
export default {
  ...
  methods: {
    ...
    clearFilter() {
      ...
      // this.$router.push({path: this.$changeRoute('/catalog'), query})
      this.costFrom = ''
      this.costTo = ''
