<template lang="pug">
  .section-search__list(v-if="CATEGORIES")
    .section-search__list-wrap.flex(:class="{'css-scrollbar': !$screen.st}")
      item-category.section-search__item(:text="PAGE_HOME.allCategories" @click.prevent.native="openCategory(`/catalog`)")
      .section-search__item(v-for="(item, i) in CATEGORIES" :key="i")
        ...
        tippy(theme="tippy-category" placement="bottom-center" v-else )
          ...
          .sub-categories(:class="{'sub-categories__big': item.allSubCategories.length > 2}")
            button-primary.sub-categories-name.strong(v-for="(itemSub, j) in item.allSubCategories" :key="j" :to="`/catalog/${item.slug}/${itemSub.slug}`") {{itemSub.name}}
  ...
  methods: {
    ...
    openCategory(item, index) {
      if (!this.$screen.lt) {
        ...
        this.$router.push(this.$changeRoute(`/catalog/${item.slug}`))
      }
    }
  }
}
