<template lang="pug">
  .item-chat(:class="Mods")
    form-checkbox.item-chat__checkbox(v-if="checkbox" :checked="checked" @change.native="$emit('changeCheckbox')")
    ...
    @click="$emit('change',conversation_id)"
      ...
      span.item-chat__avatar(v-if="avatar")
        set-img(img-class="object-fit" :src="avatar" :default-img="avatar")
      ...
      span.item-chat__info
        ...
        span.item-chat__body
          span.item-chat__content
            component.item-chat__desc(:is="isClickable ? 'NuxtLink' : 'span'" :to="isClickable ? slug : false" v-if="desc") {{desc}}
            span.item-chat__text
              span.item-chat__text-inner
                slot
              span.chat-message__read(:class="{'is-read':last_message.is_seen}" v-if="isSender")
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isSender() {
        // if (Object.keys(this.last_message).length > 0){
        // }
    ...
    beforeDestroy() {
      if (this.ACTIVE_CONVERSATION_ID) this.disconnect(this.ACTIVE_CONVERSATION_ID)
    },
    created() {

      if (this.conversation_id) {
        this.$echo.private(`chat-conversation-readAt-all.${this.conversation_id}-${this.$auth.user.id}`)
            .listen('.chat-conversation-readAt-all', (res) => {
              console.log('read_all', res);
              // this.CHANGE_SIDEBAR_WHEN_SEND_MESSAGE(res.message);
