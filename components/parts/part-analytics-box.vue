    ...
    swiper.analytics-box__slider( :options="SwiperOptions" :disabled="true")
      swiper-slide.analytics-box__slide(
        ...
        .analytics-box__inner
          ...
          part-statistic.analytics-box__statistic(v-if="slide.showText" :class="{'opacity-0' : !slide.statistics}" :statistics="slide.statistics")
        .analytics-box__img(:class="{'cover-img' : slide.img}")
          set-img(:src="slide.img && slide.img.length !== 0 ? slide.img[0] : ''" :defaultImg="require(`~/assets/img/analytics-box__img.jpg`)")
      template(v-slot:nav)
