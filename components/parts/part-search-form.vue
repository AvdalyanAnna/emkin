<template lang="pug">
  form.search-form( @submit.prevent="searchProducts")
    .search-form__row
      ...
      .search-form__field
        ...
        div(v-if="search.length > 2 && searchListOn ")
          .search-form__list.css-scrollbar(v-if="searchList && searchList.data &&  searchList.data.length > 0 " v-click-outside="closeList")
            .search-form__item(
              v-if="searchList.data && searchList.data.length > 0"
              v-for="(item, i) in searchList.data"
              :key="i.id"
            )
              span.search-form__link(  @click="changePage(getRouter(item),{search_text:item.product_name})")
                span
            ...
            .search-form__item(
              ...
              span.search-form__link(@click="changePage($changeRoute(`/catalog`),{search_text:item.product_name})" )
                span
                  span.search-form__link-name
                    | {{ item.product_name }}
            ...
            .search-form__item(
              ...
              span.search-form__link(@click="changePage($changeRoute(`/catalog`),{search_text:search})" )
                span
                  span.search-form__link-name
                    | All Categories
...
export default {
  ...
  methods: {
    ...
    getRouter(item) {
      ...
      return this.$changeRoute(`/catalog/${slug}`);
    },
    changePage(path, obj) {
      this.$router.push(`${path}?${encodeQuery(obj)}`)
    }
  },
}
</script>
