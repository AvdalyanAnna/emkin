  ...
  .create-gallery
    .create-gallery__preview
      .create-gallery__action
        button-medium.create-gallery__action-btn.create-gallery__action-btn_main(v-if="active !== 0 " big borderWhite icon="pinned" type="button" @click.prevent.native="toTop") {{content.pin}}
      ...
      .create-gallery__preview-img(v-if="thumbnails.length > 0")
        set-img(img-class="object-fit" :src="thumbnails[active]")
      ...
      draggable.create-gallery__thumbnails-list.flex(
        ...
        .create-gallery__thumbnails-item.canDrag(
          ...
          item-photo-thumbnail.create-gallery__thumbnails-btn(
            ...
            :class="[{isActive:active === i},{error:item && item.size > 2500000}]"
          )
