<template lang="pug">
	.select(:class="Mods")
		...
		.select__wrap
			input.select__input(type="hidden" :name="name" placeholder="test" :value="value")
			.select__head(v-click-outside="closeList" @click="toggleList")
				.select__value {{ value && value.refusal  ? value.refusal :'Selected' }}
				.select__arrow
					...
					.select__list
						.select__item(v-for="(item, i) in items" :key="i")
							label.select__btn
								...
								span.select__btn-text {{ item.refusal }}
					slot
